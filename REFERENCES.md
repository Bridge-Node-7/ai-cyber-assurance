
# References

Authoritative public references supporting the repository's assurance, engineering, AI, incident-response, Zero Trust, secure-development, and public-safety approach.

Publication status should be rechecked before each major release.

## Cybersecurity Risk and Systems Engineering

### NIST Cybersecurity Framework 2.0

- **NIST CSWP 29**, *The NIST Cybersecurity Framework (CSF) 2.0*
- Status: Final
- Published: February 2024
- Source: https://csrc.nist.gov/pubs/cswp/29/the-nist-cybersecurity-framework-csf-20/final
- Relevance: Govern, Identify, Protect, Detect, Respond, and Recover outcomes.

### NIST SP 800-160 Vol. 1 Rev. 1

- *Engineering Trustworthy Secure Systems*
- Status: Final
- Published: November 2022
- Source: https://csrc.nist.gov/pubs/sp/800/160/v1/r1/final
- Relevance: Systems security engineering, trustworthiness, lifecycle, and assurance.

### NIST SP 800-160 Vol. 2 Rev. 1

- *Developing Cyber-Resilient Systems: A Systems Security Engineering Approach*
- Status: Final
- Published: December 2021
- Source: https://csrc.nist.gov/pubs/sp/800/160/v2/r1/final
- Relevance: Anticipate, withstand, recover, and adapt.

### NIST SP 1308

- *NIST Cybersecurity Framework 2.0: Cybersecurity, Enterprise Risk Management, and Workforce Management Quick-Start Guide*
- Status: Final
- Published: March 2026
- Source: https://csrc.nist.gov/pubs/sp/1308/final
- Relevance: Connecting cybersecurity outcomes to enterprise risk decisions.

## Cryptographic Agility and Post-Quantum Migration

### NIST CSWP 39upd1

- *Considerations for Achieving Crypto Agility: Strategies and Practices*
- Status: Final
- Published: December 2025; updated June 2026
- Source: https://csrc.nist.gov/pubs/cswp/39/upd1/considerations-for-achieving-crypto-agility/final
- Relevance: Cryptographic agility, transition planning, governance, inventory, testing, and operational change.

### NIST NCCoE Migration to Post-Quantum Cryptography

- Project: *Migration to Post-Quantum Cryptography*
- Status: Active NCCoE project
- Source: https://www.nccoe.nist.gov/applied-cryptography/migration-to-pqc
- Relevance: Cryptographic discovery, migration planning, interoperability, and controlled nonproduction transition testing.

## Identity and Zero Trust

### NIST SP 800-207

- *Zero Trust Architecture*
- Status: Final
- Published: August 2020
- Source: https://csrc.nist.gov/pubs/sp/800/207/final
- Relevance: Identity, resources, policy enforcement, continuous evaluation, and least privilege.

## Incident Response

### NIST SP 800-61 Rev. 3

- *Incident Response Recommendations and Considerations for Cybersecurity Risk Management: A CSF 2.0 Community Profile*
- Status: Final
- Published: April 2025
- Source: https://csrc.nist.gov/pubs/sp/800/61/r3/final
- Relevance: Integrating incident response into cybersecurity risk management.

## Secure Software and AI Development

### NIST SP 800-218

- *Secure Software Development Framework (SSDF) Version 1.1: Recommendations for Mitigating the Risk of Software Vulnerabilities*
- Status: Final
- Published: February 2022
- Source: https://csrc.nist.gov/pubs/sp/800/218/final
- Relevance: Secure development practices and evidence.

### NIST SP 800-218A

- *Secure Software Development Practices for Generative AI and Dual-Use Foundation Models: An SSDF Community Profile*
- Status: Final
- Published: July 2024
- Source: https://csrc.nist.gov/pubs/sp/800/218/a/final
- Relevance: Generative AI and foundation-model development practices.

### NIST SP 800-218 Rev. 1 / SSDF 1.2

- *Secure Software Development Framework (SSDF) Version 1.2: Recommendations for Mitigating the Risk of Software Vulnerabilities*
- Status: Initial Public Draft
- Published: December 2025
- Source: https://csrc.nist.gov/pubs/sp/800/218/r1/ipd
- Relevance: Draft evolution of SSDF. Do not treat it as a final replacement for SSDF 1.1.

## AI Risk Management

### NIST AI 100-1

- *Artificial Intelligence Risk Management Framework (AI RMF 1.0)*
- Status: Final
- Published: January 2023
- Source: https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10
- Relevance: Govern, Map, Measure, and Manage AI risks.

### NIST AI 600-1

- *Artificial Intelligence Risk Management Framework: Generative Artificial Intelligence Profile*
- Status: Final
- Published: July 2024
- Source: https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-generative-artificial-intelligence
- Relevance: Generative AI risk considerations and actions.

## Secure by Design

### CISA Secure by Design

- Source: https://www.cisa.gov/securebydesign
- Relevance: Secure defaults, producer responsibility, transparency, and vulnerability handling.

## OWASP Generative AI Security

### OWASP Top 10 for LLM Applications / GenAI Security Project

- Project: https://genai.owasp.org/
- 2025 risks: https://genai.owasp.org/llm-top-10/
- Relevance: Application-level generative AI risk categories and mitigations.

## Use and Limitations

These references support alignment and terminology. This repository does not claim formal conformity, certification, authorization, legal sufficiency, or comprehensive coverage of any referenced framework.

System owners remain responsible for selecting current, applicable, authoritative requirements for their jurisdiction, sector, mission, architecture, and risk.
